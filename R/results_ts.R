...
get_renomarlized_features <- function(results, populations) {
  ...
  for (i in 1:res.count) {
    ...
    best <- results[[i]]$best
    for (item in names(results[[i]])) {
      if (!(item %in% (c("accept.tot", "best", "transforms", "transforms.ts")))) results[[i]][[item]] <- results[[i]][[item]][pops.use[[i]]]
    }
    results[[i]]$accept.tot <- accept.tot
    results[[i]]$best <- best
  }
  ret_list <- list(features = features, renorms = renorms, results = results, pw = pw)
  return(ret_list)
}
...
get_best_results <- function(results) {
  ...
  ret.list <- list(bests = bests, pop.best = pop.best, crit.best = crit.best, thread.best = thread.best)
  return(ret.list)
}
...
# row 1-3 are the simplest equivalent features based on three different complexity measures
# row 4 is the total weighted density of those features
get_feats.map <- function(mock.data.precalc, features, renorms, tol) {
  # Calculate the correlation to find equivalent features
  cors <- cor(mock.data.precalc)
