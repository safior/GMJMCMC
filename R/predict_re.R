...
predict.gmjmcmc.inla <- function(object, data, lookback_window, link = function(x) x, quantiles = c(0.025, 0.5, 0.975),  pop = NULL, tol =  0.0000001, ...) {
  object <- merge_results.re(list(object), populations = pop, tol = tol)

  n.row <- nrow(data) - lookback_window
  n.pred <- sum(is.na(data[, 1]))
  #print(data)
  #print(n.row)
  #print(n.pred)
  #print((n.row-n.pred+1))
  ...
  for (i in seq_along(object$results)) {
    ...
    for (j in seq_along(object$results[[i]]$populations)) {
      ...
      yhat <- matrix(0, nrow=n.pred, ncol=length(models))
      ...
      for (k in seq_along(models)) {
        ...
        #print(inla.mod$summary.fitted.values)
        #print(inla.res[(n.row-n.pred+1):n.row, 'mean'])
        yhat[, k] <- inla.mod$summary.fitted.values[(n.row-n.pred+1):n.row, 'mean']
      }
...
predict.gmjmcmc.nlme <- function (object, x, re.data, link = function(x) x, quantiles = c(0.025, 0.5, 0.975),  pop = NULL,tol =  0.0000001, ...) {
  ...
  merged <- merge_results.re2(list(object), data = cbind(1,x), populations = pop,tol = tol)#, lw = object$lookback_window)
  ...
  for (i in seq_along(object$results)) {
    ...
    for (j in seq_along(object$results[[i]]$populations)) {
      ...
      for (k in seq_along(models)) {
        ...
        yhat[, k] <- predict(nlme.mod, new.data)
      }
