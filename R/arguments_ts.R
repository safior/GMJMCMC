...
gen.probs.gmjmcmc.ts <- function (ts.transforms, transforms) {
  ...
  # Transformation probabilities. Time series transformation correponds to the last placement
...
gen.params.gmjmcmc.ts <- function(data) {
  ...
  # Above line is not necessarily true, but should not really matter since it is only used below.
  ncov <- ncol(data) - 1
